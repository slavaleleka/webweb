/*! extended-css - v2.0.0 - Fri Aug 26 2022
...
var BrowserstackTest = (function (exports) {
    ...
    const isTargetBrowser = target => {
      var _navigator$userAgentD;

      // let uaStringName = null;
      let uaMarkers = null;
      let brand = null;

      if (target === CHROME_BROWSER_NAME) {
        // uaStringName = 'Chrome';
        uaMarkers = ['Chrome'];
        brand = 'Google Chrome';
      } else if (target === FIREFOX_BROWSER_NAME) {
        // uaStringName = 'Firefox';
        uaMarkers = ['Firefox'];
      } else if (target === EDGE_BROWSER_NAME) {
        // uaStringName = 'Edg';
        uaMarkers = ['Edg'];
      } else if (target === SAFARI_BROWSER_NAME) {
        // uaStringName = 'Safari';
        uaMarkers = ['Version', 'Safari'];
      } // if (!uaStringName) {
      ...
      if (!brandsData || !brand) {
        // return navigator.userAgent.includes(uaStringName);
        return uaMarkers.every(marker => navigator.userAgent.includes(marker));
    ...
    const getCurrentSupportedBrowserData = () => {
      const currentBrowserData = SUPPORTED_BROWSERS.find(_ref => {
        let {
          NAME
        } = _ref;
        return isTargetBrowser(NAME);
      });
      return currentBrowserData || null;
    };
    /**
    ...
    const getBrowserVersion = browserName => {
      ...
      let brand = null;
      let uaStringMask = null;

      if (browserName === CHROME_BROWSER_NAME) {
        brand = 'Google Chrome'; // avoid Chromium Edge match

        uaStringMask = /\sChrome\/(\d+)\..+\s(?!.*Edg\/)/;
      } else if (browserName === FIREFOX_BROWSER_NAME) {
        uaStringMask = /\sFirefox\/(\d+)\./;
      } else if (browserName === EDGE_BROWSER_NAME) {
        uaStringMask = /\sEdg\/(\d+)\./;
      } else if (browserName === SAFARI_BROWSER_NAME) {
        uaStringMask = /\sVersion\/(\d+)\..+\sSafari\//;
      }
      ...
      const brandsData = (_navigator$userAgentD2 = navigator.userAgentData) === null || _navigator$userAgentD2 === void 0 ? void 0 : _navigator$userAgentD2.brands;
    ...
    const isBrowserSupported = () => {
      ...
      const supportedData = getCurrentSupportedBrowserData();

      if (!supportedData) {
        // allow execute ExtendedCss if it is not unsupported
        return true;
      }

      const browserVersion = getBrowserVersion(supportedData.NAME);

      if (!browserVersion || browserVersion < supportedData.MIN) {
        return false;
      }
    ...
    class ExtMutationObserver {
      ...
      constructor(callback) {
        ...
        this.observer = new natives.MutationObserver(callback);
        this.isActive = false;
