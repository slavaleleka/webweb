/*! extended-css - v2.0.0 - Sat Aug 27 2022
...
var BrowserstackTest = (function (exports) {
  ...
  const getUserAgentBrand = () => {
    ...
    const chromiumBrand = brandsData.find(brandData => brandData.brand === CHROMIUM_BRAND_NAME);
    return chromiumBrand || getLast(brandsData);
  };
  ...
  const parseUserAgent = () => {
    ...
    const browserNames = Object.keys(SUPPORTED_BROWSERS_DATA);

    for (let i = 0; i < browserNames.length; i += 1) {
      const match = SUPPORTED_BROWSERS_DATA[browserNames[i]].MASK.exec(userAgent);

      if (match) {
        if (match[2] === SAFARI_BROWSER_NAME) {
          // because of regexp
          browserName = match[2];
  ...
  const getCurrentBrowserInfoAsSupported = () => {
    const brandData = getUserAgentBrand();
    ...
    const browserName = brand === 'Chromium' || brand === 'Google Chrome' ? CHROME_BROWSER_NAME : brand;
    return {
      browserName,
